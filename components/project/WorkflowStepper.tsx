...
export function WorkflowStepper({ stages, currentStage, completedStage, onStageClick }: WorkflowStepperProps) {
  const getStageStatus = (stageId: number) => {
    ...
    if (stageId === completedStage + 1) return 'next';
    return 'locked';
  };
  ...
  const getStageColors = (stageId: number) => {
    ...
    switch (status) {
      ...
      case 'locked':
        return 'bg-gray-500/20 text-gray-500 border-gray-500/30 cursor-not-allowed';
    }
  };

  const canClickStage = (stageId: number) => {
    return stageId <= completedStage + 1;
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-center space-x-6 text-sm text-gray-400 mt-6">
        ...
        <div className="flex items-center space-x-2">
          <div className="w-4 h-4 rounded-full bg-gray-500/20 border border-gray-500/30 flex items-center justify-center">
